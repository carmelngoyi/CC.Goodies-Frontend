...
async function loadProducts() {
  const res = await apiFetch("/Api/poultry");
...
const Poultry = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${elasticIP}/poultry`);
  ...
  if (loading) return <p>Loading Poultry products...</p>;
  if (error) return <p>Error: {error}</p>;

  return (
    <>
  <div className="page-wrapper6">
    ...
    <div className="divider">
      <h1 className="divider-big">HIGH</h1>
      <h1 className="divider-text text1">MEATY!</h1>
      <h1 className="divider-text text2">JUICY!</h1>
      <h1 className="divider-text text1">PROTEIN!</h1>
      <h1 className="divider-big">QUALITY</h1>
    ...
    <h1 style={{fontFamily: "Arial, Helvetica, sans-serif",
      ...
      color: "darkblue", 
      textAlign: "center"}} 
      className="page-title">High-Quality Poultry for Every Recipe!</h1>
